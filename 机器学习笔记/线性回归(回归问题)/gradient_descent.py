...
def linear_regression(train, label_train, a, test, label_test):
    Q = np.zeros(([NUM + 1, 1]))
    length = train.shape[0]
    for i in range(100000):
        cost = np.matmul(train, Q) - np.transpose([label_train])
        if i % 1000 == 0:
            print('********** i = ' + str(i))
            c = np.matmul(np.transpose(cost), cost)
            c = np.true_divide(c, 2 * length)
            #print(c)
        cost = np.matmul(np.transpose(train), cost) * a
        Q = Q - np.true_divide(cost, length)
    return Q
...
def main():
    ...
    Q1 = linear_regression(train, label_train, 0.0001, test, label_test)
    train, test, label_train, label_test = read(10)
    Q2 = linear_regression(train, label_train, 0.0001, test, label_test)
